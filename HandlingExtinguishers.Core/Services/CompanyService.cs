                             ...
                             IRepositoryEmployee repositoryEmployee ) : ICompanyService
{
    ...
    public async Task<FilterCompanyResponse> SearchCompanies( QueryParameter filter )
    {
        try
        {
            var response = new FilterCompanyResponse();
            ...
            var result = mapper.Map<List<CompanyResponse>>( pagegResult.Resource );

            response.Companies = PaginationHelper.CreatePagedReponse<CompanyResponse>( result, filter, pagegResult.TotalRecords );
    ...
    public async Task<FilterCompanyResponse> SearchCompaniesDisabled( QueryParameter filter )
    {
        try
        {
            var response = new FilterCompanyResponse();
            ...
            var result = mapper.Map<List<CompanyResponse>>( pagedResult.Resource );

            response.Companies = PaginationHelper.CreatePagedReponse<CompanyResponse>( result, filter, pagedResult.TotalRecords );
    ...
    {
        var result = await repositoryCompany.FindByAsNoTracking( company => company.Active && company.CompanyId == companyId ).FirstOrDefaultAsync();

        if ( result is not null )
        {
            return mapper.Map<CompanyResponse>( result );
        }
        else
        {
    ...
    public async Task<CompanyRequest> CreateCompany(CompanyRequest company )
    {
        try
        {
            ...
            var response = mapper.Map<CompanyRequest>( result );
    ...
    public async Task<CompanyRequest> UpdateCompany(Guid companyId, PatchCompanyRequest request)
    {
        try
        {
            var search = await repositoryCompany.FindBy(x => x.CompanyId == companyId).FirstOrDefaultAsync();
    ...
    {
        var company = await repositoryCompany.FindBy( company => company.CompanyId == companyId ).FirstOrDefaultAsync();
